...
Page({
  ...
  initLocalAccountsGroup() {
    ...
    if (Array.isArray(accounts)) {
      ...
      this.setData({
        localList: this.orderGroups(localList),
        localListInited: true
      })
    }
  },
  // 云端账号
  ...
  setUnderlineLeft() {
    ...
    if (dataType === 0) {
      this.setData({
        underlineLeft: this.data.cloudLeft
      })
    }
    if (dataType === 1) {
      this.setData({
        underlineLeft: this.data.localLeft
      })
    }
    this.setData({
      initedUnderline: true
    })
  },
  // 触发页面更新
